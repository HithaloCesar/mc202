...
char operate(int N, int M[][MAX][MAX]) {
	char operation[MAX_OPERATION_SIZE];
	scanf(" %s", operation);
	char x, y;
	scanf(" %c %c", &x, &y);
	int (*X)[MAX] = choose_matrix(N, M, x);
	int (*Y)[MAX] = choose_matrix(N, M, y);
...
int main(void) {
	...
	int O;
	scanf("%d", &O);
	int M[MAX_MATRICES][MAX][MAX];
	for (int m = 0; m < 2; m++)
		stripe_v(M[m], N, 3, 1);
	for (int i = 0; i < O; i++) {
		printf("\n");
		char id = operate(N, M);
		int (*result)[MAX] = choose_matrix(N, M, id);
