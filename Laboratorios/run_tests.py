...
def get_files(lab_n: str) -> Tuple[str, str, List[str]]:
    labs = listdir(".")
    lab_dir = [file for file in labs if file.startswith(f"Lab_{lab_n}")].pop()
    test_dir = "Testes"
...
def print_diff(lab_dir, test_dir, test_n):
    print(f"Diferenças no teste {test_n}:\n")
    var = "diff " + path.join(f"\"{lab_dir}", test_dir, f"{test_n}-c.out\"") + " " + path.join(f"\"{lab_dir}", test_dir, f"{test_n}_ans.out\"")
    system(var)
    print()
...
def main() -> None:
    ...
    for test_n, test_in in enumerate(in_files, 1):
        ...
        print_diff(lab_dir, test_dir, test_n)
